...
class FormItems extends React.PureComponent {
  getItem = item => {
    switch (item.type) {
      ...
      case 'dateRange':
        return <RangePicker showTime={{ format: 'HH:mm' }} format="YYYY-MM-DD HH:mm" placeholder={item.placeholder || ['Start Time', 'End Time']} onChange={item.onChange} onOk={item.onOk} allowClear={item.allowClear} />

      case 'radio':
        return (
          <RadioGroup disabled={item.disabled}>
  {item.radio.map(radio => <Radio value={radio.value} key={radio.value}>{radio.text}</Radio>)}
</RadioGroup>
        )

      case 'select':
        return (
          <Select disabled={item.disabled} onChange={item.onChange} showSearch={item.showSearch} optionFilterProp={item.optionFilterProp} filterOption={item.filterOption} allowClear={item.allowClear}>
  {item.options.map(option => {
      ...
      case 'number':
        return <InputNumber min={item.min} max={item.max} step={item.step} onChange={item.onChange} />

      case 'btn':
        return <Button type={item.btnType || 'primary'} loading={item.loading} onClick={item.onClick} disabled={item.disabled}>{item.text}</Button>

      case 'search':
        return <Search placeholder={item.placeholder} enterButton={item.enterButtonText} size={item.size || 'large'} onSearch={item.onSearch} />

      case 'check':
        return <Checkbox.Group>{item.box.map((box, i) => <Checkbox value={box.value} key={i}>{box.text}</Checkbox>)}</Checkbox.Group>
      case 'component':
...
return formItems.map((item, i) => {
        ...
        if (required) rules = [...rules, { required: true, message: `please add the ${label}.` }]
        
return (
          <span className="item-wrapper" key={i}>
{toggler ? <Form.Item label={label} key={i} extra={extra} className={item.className}>
  ...
  render() {
    return (
      <div className="form-wrapper">
        {this.getForm(this.props.formItems)}
      </div>
    )
  }
}
